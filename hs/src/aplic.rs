...
pub type AplicMode = crate::imsic::PrivMode;
...
impl Aplic {
    ...
    fn set_sourcecfg(&mut self, irq: u32, mode: SourceModes) {
        assert!(irq > 0 && irq < 1024);
    ...
    pub fn get_ip(&mut self, irq: u32) {
        assert!(irq > 0 && irq < 1024);
        let irqidx = irq as usize / 32;
        println!("aplic eip: 0x{:08x}",self.setip[irqidx] ) ;   
    }
}
...
pub fn aplic_init(hartid:usize) {
    ...
    // has been enabled.
    splic.set_target_msi(10, hartid_u32, 1, 10);
